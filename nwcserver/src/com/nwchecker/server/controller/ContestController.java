...
public class ContestController {
	...
	public String getContestsByStatus(
		...
		if ((principal == null)
			...
			if (status == null) {
				...
				count = contestService.getPageCount(
						Contest.Status.stringToStatus(status), pageSize);
				allContests = contestService.getPagedContestsByStatus(
						Contest.Status.stringToStatus(status), pageSize, page);
			}
		} else {
			...
			if (status == null) {
				count = contestService.getUserEntryCount(user.getUserId(),
						pageSize);
				allContests = contestService.getUserPagedContests(pageSize,
						page, user);
			} else {
				count = contestService.getUserEntryCountByStatus(
						user.getUserId(), pageSize,
						Contest.Status.stringToStatus(status));
				allContests = contestService.getUserPagedContestsByStatus(
						pageSize, page, user,
						Contest.Status.stringToStatus(status));
			}
